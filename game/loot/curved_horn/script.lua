function CurvedHorn_enter(host, me, owner)
...
function CurvedHorn_trigger1_check(host, me)
    local roll = Common_LastRoll(host)
    return roll.isCombatRoll
end

function CurvedHorn_trigger1(host)
...
function CurvedHorn_trigger2_check(host, me)
    return Common_OwnersTurn(host, me.id)
end

function CurvedHorn_trigger2(host)
    local me = this(host)
    CardData[me.id].rolls = CardData[me.id].rolls + 1
end
